...
function get_api_connection_parameters($api_config_file_path){
	...
	$config['API_KEY_FAKTURA'] = Configuration::get('ifirma_api_key_faktura');
	$config['API_KEY_ABONENT'] = Configuration::get('ifirma_api_key_abonent');
	$config['API_LOGIN'] = Configuration::get('ifirma_api_login');
...
function get_order_details($cart_order_id){
	...
	$order_details = DB::getInstance()->execute($sql);
...
function faktura_stworzona($cart_order_id){
	...
	$res = DB::getInstance()->execute($db_query); 
	return $res[0]['ile'];
}
function add_info_about_generated_document($document_type,$cart_order_id,$invoice_number,$invoice_type){
	global $glob;
	DB::getInstance()->execute("insert into ifirma_invoice_map(document_type,cart_order_id,invoice_number,invoice_type)  values('$document_type','$cart_order_id','$invoice_number','$invoice_type')" );
	
}
function add_info_about_invoice_fault($cart_order_id){
	DB::getInstance()->execute("update ifirma_invoice_map set correction_needed = 1 where cart_order_id = '$cart_order_id'");
}
function add_info_about_invoice_fault_resolved($cart_order_id){
	DB::getInstance()->execute("update ifirma_invoice_map set correction_done = 1 where cart_order_id = '$cart_order_id'");
}
function needs_attention($cart_order_id){
	...
	$niepoprawiona = DB::getInstance()->execute($db_query); 
